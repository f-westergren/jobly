...
describe('GET /jobs', () => {
  ...
  test('get a list of jobs with min_salary and min_equity parameters', async () => {
    const res = await request(app).get('/jobs').send({min_salary: 26000, min_equity: 0.4})
    expect(res.statusCode).toBe(200)
    expect(res.body).toEqual({ jobs: [testJob3] })
  })

  test('get a list of jobs mith search string', async () => {
    const res = await request(app).get('/jobs').send({search: 'test2'})
    expect(res.statusCode).toBe(200)
    expect(res.body).toEqual({ jobs: [testJob2] })
  })

  test('get an empty list of jobs mith search string', async () => {
    const res = await request(app).get('/jobs').send({search: 'NoJob'})
    expect(res.statusCode).toBe(200)
...
describe('POST /jobs', () => {
  test('add a new job', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(res.body).toEqual({ job: jobTest })

    // Make sure job was created
    const getJobRes = await request(app).get(`/jobs/${jobTest.id}`)
    expect(getJobRes.body.job).toEqual(jobTest)
  })
})
...
describe('GET /jobs/:id', () => {
  test('gets a single job', async () => {
    const res = await request(app).get(`/jobs/${testJob4.id}`)
    expect(res.statusCode).toBe(200)
...
describe('PATCH /jobs/:id', () => {
  test('updates a single job', async () => {
    ...
    expect(res.statusCode).toBe(200)
    expect(res.body).toEqual({ job: updateData })

    // Check updated company data
    const getJobRes = await request(app).get(`/jobs/${testJob4.id}`)
...
describe('DELETE /jobs/:id', () => {
  test('Deletes a single job', async () => {
    const res = await request(app).delete(`/jobs/${testJob4.id}`)
    expect(res.statusCode).toBe(200)
